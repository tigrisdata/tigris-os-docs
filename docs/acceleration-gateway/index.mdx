...
<!-- NOTE: The SVG below is rendered from TAGOverviewDiagram. The mermaid
notation is kept here as a machine-readable description for AI agents.
flowchart TB
    subgraph inst["Training Instance"]
        TC[Training Code] - - >|S3 API| TAG[Acceleration Gateway]
        TAG - - - NV[Local NVMe Cache]
    end
    NV -.->|cache miss| TOS[Tigris Object Storage]
    TOS - - >|fill cache| NV
-->
...
TAG implements the S3 REST API for commonly used Tigris operations. Clients
interact with TAG using standard AWS SDKs, the AWS CLI, or any S3-compatible
library. The only change is the endpoint URL.
...
In default mode, TAG preserves the client's original AWS SigV4 Authorization
header and forwards requests to Tigris as-is. TAG adds cryptographically signed
proxy headers so Tigris can validate both the client's identity and TAG's
identity in a single round-trip.

:::info[Security]

TAG never sees or stores client secret keys. Clients authenticate directly with
Tigris using their own credentials.

:::

### Embedded Cache with RocksDB

Each TAG node runs an embedded cache backed by RocksDB, optimized for NVMe
storage. Objects are stored using a two-key pattern:
...
Separating metadata from body enables efficient HEAD requests and conditional
operations without reading the full object.
...
<!-- NOTE: The SVG below is rendered from TAGCoalescingDiagram. The mermaid
notation is kept here as a machine-readable description for AI agents.
flowchart LR
    A[Client A] - - > T[TAG coalescer]
    B[Client B] - - > T
    C[Client C] - - > T
    D[Client D] - - > T
    T - - >|single upstream GET| U[Tigris]
    U - - >|stream chunks| T
    T -.-> A & B & C & D
-->
...
<!-- NOTE: The SVG below is rendered from TAGClusterDiagram. The mermaid
notation is kept here as a machine-readable description for AI agents.
flowchart TB
    LB[Load Balancer] - - > N1[TAG-1 keys A-F]
    LB - - > N2[TAG-2 keys G-R]
    LB - - > N3[TAG-3 keys S-Z]
    N1 <-- >|gRPC| N2
    N2 <-- >|gRPC| N3
    N1 <-.gossip.-> N2
    N2 <-.gossip.-> N3
...
Nodes discover each other via gossip, consistent hashing spreads keys across the
cluster, and gRPC forwards requests for remote keys transparently. You don't
need to manually assign keys or configure sharding.

## Architecture

Here's how a request flows through TAG's internals:

<!-- NOTE: The SVG below is rendered from TAGArchitectureInternalsDiagram.
The mermaid notation is kept here as a machine-readable description for AI agents.
flowchart LR
    C[S3 clients SDK/CLI/boto3] - - > H[HTTP server :8080]
    H - - > A[SigV4 auth]
    H - - > P[Proxy service]
    P - - > R[Request coalescing]
    P - - > G[Range optimization]
    P - - > K[(RocksDB cache)]
    K - - > D[NVMe disk]
    P -.-> U[Tigris Object Storage]
    P - - > X[gRPC :9000]
    P - - > M[Gossip :7000]
-->
...
[Configuration Reference](./configuration#cluster-mode) for details.
