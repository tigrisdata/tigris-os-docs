...
<!-- NOTE: The SVG below is rendered from TAGFirstRequestDiagram. The mermaid
...
sequenceDiagram
    ...
    Note over T: Unwrap and store derived keys
...
<!-- NOTE: The SVG below is rendered from TAGSubsequentRequestDiagram. The mermaid
notation is kept here as a machine-readable description for AI agents.
sequenceDiagram
    participant C as Client
    participant T as TAG
...
<!-- NOTE: The SVG below is rendered from TAGAccessControlDiagram. The mermaid
notation is kept here as a machine-readable description for AI agents.
flowchart TD
    R[Request arrives at TAG] - - > A{Authorization header?}
    A - - >|Missing| F1[Forward to Tigris]
    A - - >|Malformed| REJ[Reject with 4xx]
    A - - >|Present| K{Access key known?}
    K - - >|No| F2[Forward to Tigris, learn keys on success]
    K - - >|Yes| S{SigV4 valid?}
    S - - >|Mismatch| F3[Forward to Tigris, re-learn keys if stale]
    S - - >|Valid| AZ{Authz cache hit?}
    AZ - - >|Miss / expired| F4[Forward to Tigris, re-authorize on success]
    AZ - - >|Hit| OK[AuthValidated, serve from cache]
-->
