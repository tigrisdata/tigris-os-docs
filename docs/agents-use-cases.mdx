...
![Persist agent memory globally](/img/use-cases/agents-persist-memory.excalidraw.png)
...
For more information, see the docs: [Bucket locations](/docs/buckets/locations/)
...
Global buckets are
[strongly consistent within a region](/docs/concepts/consistency/) and
...
can restart them after failures or migration.
[Storing snapshots in Tigris](/docs/buckets/snapshots-and-forks/) makes them
available from the nearest region to whichever machine picks up the resumed
agent, with no cross-region prefetch and no egress cost on the read. Agents that
need filesystem semantics can mount the bucket directly via
[TigrisFS](/docs/training/tigrisfs/) and see a plain `/data` directory.

![Checkpoint and resume agents](/img/use-cases/agents-checkpoint.excalidraw.png)
...
![Trigger processing pipelines](/img/use-cases/agents-pipelines.excalidraw.png)
...
![Fork storage for isolated eval runs](/img/use-cases/agents-fork-eval.excalidraw.png)
...
![Distribute agent artifacts](/img/use-cases/agents-distribute.excalidraw.png)
...
![Provision isolated storage for sandboxes](/img/use-cases/agents-sandboxes.excalidraw.png)
