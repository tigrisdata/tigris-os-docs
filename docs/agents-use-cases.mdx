...
<Section>
  ...
  <Card
    ...
    to="#provision-isolated-storage-for-ephemeral-sandboxes"
    icon="img/cube"
  />
</Section>

## 1. Persist agent memory globally {#persist-agent-memory-globally}
...
A [global bucket](/docs/buckets/locations/) serves every read from the nearest
replica at no egress cost, so those read-heavy loops don't accumulate transfer
fees regardless of which region the agent is running in.
...
[Bucket locations](/docs/buckets/locations/) (per-object residency)
...
## 2. Checkpoint and resume agents anywhere {#checkpoint-and-resume-agents-anywhere}
...
## 3. Trigger processing pipelines on upload {#trigger-processing-pipelines-on-upload}
...
unchanged keys, and produces thundering-herd load during upload spikes.
[Tigris Object Notifications](/docs/buckets/object-notifications/) replace the
polling loop with a push model. An HTTP `POST` request fires to your webhook the
moment an object lands, carrying the bucket, key, size, and ETag needed to begin
...
## 4. Fork storage for isolated eval runs {#fork-storage-for-isolated-eval-runs}
...
scale. [Tigris forks](/docs/buckets/snapshots-and-forks/) create a copy-on-write
snapshot of a bucket instantly. Each run gets full isolation, mutations in one
fork never affect another, and the source dataset stays unchanged for replay.
...
## 5. Distribute agent artifacts to a global fleet {#distribute-agent-artifacts-to-a-global-fleet}
...
In CI, a release job requests a
[presigned `PUT` URL for a versioned key](/docs/objects/presigned/) such as
...
[Bucket locations](/docs/buckets/locations/) (per-artifact residency)

## 6. Provision isolated storage for ephemeral sandboxes {#provision-isolated-storage-for-ephemeral-sandboxes}
...
Each sandbox gets its own Tigris bucket mounted via
[TigrisFS](/docs/training/tigrisfs/). Your agents read and write files under the
mount point while your platform maps that to buckets and snapshots behind the
scenes. The agent sees a plain `/data` directory while the platform stores
everything as globally distributed objects. Snapshots and forks handle mid-run
checkpoints and sub-agent fan-out.
...
| [Bucket locations](/docs/buckets/locations/)                | Details on available regions, replication behavior, and how to choose locations.           |
