...
:::tip
This single file configures credentials, the LLM provider, and tells Cognee to store everything on Tigris instead of the local filesystem. Call `load_dotenv()` before any Cognee imports so these variables are available.
:::
...
async def feed_documents():
    ...
    await cognee.add("./research-paper.pdf")
...
:::tip
When building multi-agent or multi-user systems, set `ENABLE_BACKEND_ACCESS_CONTROL=True`. Cognee will create separate databases for each user and dataset on Tigris, so agents can't read each other's memory. This is the right default for any production system where different users or agent instances should have distinct knowledge bases. No extra configuration needed beyond the flag.
:::
...
**Authentication errors** -- Verify `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` are set. Tigris keys start with `tid_` and `tsec_`. If you're copy-pasting from the dashboard, check for accidental whitespace.

**Region errors** -- Set `AWS_REGION=auto`. Tigris handles routing automatically and does not require a real AWS region name.

**Wrong S3 endpoint** -- Cognee uses `aws_endpoint_url` for S3-compatible services. If requests are still hitting AWS, confirm that `aws_endpoint_url=https://t3.storage.dev` is present in your `.env` file and that `load_dotenv()` is called before any Cognee imports.

**Cognee uses local storage** -- Check that `STORAGE_BACKEND=s3` is in your `.env` file. Also verify that `DATA_ROOT_DIRECTORY` and `SYSTEM_ROOT_DIRECTORY` use `s3://` URIs pointing to your Tigris bucket. Cognee loads `.env` automatically on import, but environment variables set after import won't take effect.

**Slow first query** -- The first read against cold data fetches from Tigris. Subsequent queries are faster due to edge caching. This is expected behavior and not a configuration problem.

**boto3 addressing errors** -- Tigris requires `virtual` addressing style. Pass `Config(s3={"addressing_style": "virtual"})` when using boto3 directly. LanceDB and s3fs handle this automatically.
