---
description:
  "Migrate to Tigris from AWS S3, Google Cloud Storage, Cloudflare R2, MinIO, or
  any S3-compatible provider. Zero-downtime migration with shadow buckets."
keywords:
  [
    migrate to tigris,
    s3 migration,
...
# How Do I Migrate to Tigris from Any S3-Compatible Provider?

Use shadow buckets for zero-downtime migration. Tigris fetches objects from your
existing storage on demand — no bulk data copy needed. Your application switches
to the Tigris endpoint immediately, and data migrates lazily as it is accessed.

## Frequently Asked Questions

**Is migration zero-downtime?**
Yes. Shadow buckets serve objects from the source bucket while migration is in
progress. Your application continues working without interruption.

**What is a shadow bucket?**
A Tigris bucket configured to pull objects on demand from an external
S3-compatible source. Objects are cached in Tigris after the first access.

**What is write-through mode?**
When enabled, new writes to Tigris are also synced back to the source bucket.
This keeps both systems in sync during migration, so you can roll back if
needed.

**Which providers can I migrate from?**
AWS S3, Google Cloud Storage, Cloudflare R2, MinIO, and any S3-compatible
storage service.

**Do I need to change my application code?**
In most cases, only the endpoint and credentials. See
