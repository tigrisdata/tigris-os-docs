---
description:
  "Compare S3-compatible object storage alternatives: Tigris, Cloudflare R2,
  Backblaze B2, MinIO, and AWS S3. Features, pricing, and migration paths."
keywords:
  [
...
changing the endpoint. The main differences are pricing, global distribution,
...
| Feature                 | Tigris        | AWS S3             | Cloudflare R2   | Backblaze B2    | MinIO              |
| ----------------------- | ------------- | ------------------ | --------------- | --------------- | ------------------ |
| S3 API support          | Over 90%      | Full               | Partial         | Partial         | Full               |
| Egress fees             | None          | $0.09/GB           | None            | $0.01/GB        | Self-hosted        |
| Global distribution     | Automatic     | Manual replication | No              | No              | Self-managed       |
| Endpoint                | Single global | Region-specific    | Single          | Region-specific | Self-hosted        |
| Bucket forks            | Yes           | No                 | No              | No              | No                 |
| Shadow bucket migration | Yes           | N/A                | No              | No              | No                 |
| Edge compute            | No            | Lambda@Edge        | Workers         | No              | No                 |
| Multi-cloud             | Yes           | AWS only           | Cloudflare only | B2 only         | Any infrastructure |
| Managed service         | Yes           | Yes                | Yes             | Yes             | No (self-hosted)   |
| Free tier               | 5 GB          | 5 GB (12 months)   | 10 GB           | 10 GB           | N/A                |
...
R2 is cheapest for this workload. Tigris costs $7/month more but includes
automatic global distribution and bucket forks. AWS S3 is significantly more
expensive due to egress fees.
