...
**How do I install the Tigris CLI?**
Install via npm: `npm install -g @tigrisdata/tigris-cli`. The command is
available as both `tigris` and `t3`.

**How do I authenticate?**
Run `tigris login` for browser-based OAuth, or `tigris login credentials` to
enter an access key directly. Run `tigris configure` to save credentials
permanently.

**Can I use `t3://` prefixes for paths?**
Yes. Both `t3://my-bucket/file.txt` and `my-bucket/file.txt` work for all
commands.

**Does the CLI work in scripts and CI/CD?**
Yes. Set `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY` environment variables,
or use `tigris configure` to save credentials.
