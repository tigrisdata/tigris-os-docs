...
domain name must be a valid domain or subdomain. For example, if you own domain
`example.com` and bucket `foo-bucket`, you can create a CNAME record for domain
or subdomain that points to `foo-bucket.fly.storage.tigris.dev`.

For more information see the [Custom Domain](/docs/buckets/custom-domain/)
documentation.
