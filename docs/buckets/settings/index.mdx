...
Settings → Storage Settings**. Previously the tier was fixed at creation time.
...
Only the Standard and Infrequent Access tiers support snapshots. Selecting
Archive or Instant Retrieval Archive automatically disables snapshots for the
bucket.

The storage tier is locked while snapshots are enabled. To change the tier on a
snapshot-enabled bucket, disable snapshots first (under **Storage Settings**),
then change the tier.

When moving a bucket to a snapshot-eligible tier and turning snapshots on, the
snapshot toggle is also disabled when:
