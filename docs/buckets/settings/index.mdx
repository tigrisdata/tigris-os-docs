...
![Bucket Locations](./bucket-locations.webp)
...
the [bucket locations](/docs/buckets/locations/) documentation.

## Storage Tier

Select the storage tier for the bucket based on how frequently its data is
accessed. For more information, see the [Storage Tiers](/docs/objects/tiers/)
page.

![Storage Tier](./storage-tier.webp)

### Available tiers

| Tier                      | Value         | Best for                                                                            |
| ------------------------- | ------------- | ----------------------------------------------------------------------------------- |
| Standard                  | `STANDARD`    | Frequently accessed data. High durability, availability, and performance. (Default) |
| Infrequent Access         | `STANDARD_IA` | Data accessed less often but needing rapid access when required.                    |
| Archive                   | `GLACIER`     | Long-term archiving of data with infrequent access.                                 |
| Instant Retrieval Archive | `GLACIER_IR`  | Rarely accessed, long-lived data that still needs millisecond retrieval.            |

### Changing a bucket's storage tier

The storage tier can be changed after a bucket is created under **Bucket
Settings → Storage Settings**.

Changing the tier applies only to new objects:

- **New objects** — Objects uploaded after the change are stored in the newly
  selected tier.
- **Existing objects** — Existing objects are not migrated. They remain in the
  tier they were written in until they are overwritten or re-uploaded.

You can also set the storage tier for individual objects when you upload them.
For more information, see
[Storage Tiers: Setting object tier](/docs/objects/tiers/#setting-object-tier).

### Interaction with snapshots

Snapshots can be enabled on a bucket in any storage tier, both at creation and
afterward. You can change the storage tier and turn snapshots on or off at the
same time — the tier and snapshot state are independent, with no restrictions
between them.

The snapshot toggle is disabled only when:

- The bucket is a fork (has parent buckets).
- The bucket has dependent forks (child buckets rely on its snapshot state).
...
- **Regular to Snapshot**: the bucket must not use object lifecycle rules or
  object expiration (TTL) rules, since these are not supported on
...
1. When an object is requested, it is served from your Tigris bucket if it is
   found.
2. If the object is not found in the Tigris bucket, the object will be returned
   from your source bucket and asynchronously copied to Tigris.
...
The shadow bucket must be outside Tigris. A Tigris endpoint is rejected with
`Migration source cannot be Tigris endpoint`. Use a
[bucket fork](/docs/buckets/snapshots-and-forks/) or `aws s3 sync` to copy
between Tigris buckets.
...
[Soft Delete](/docs/buckets/soft-delete/) page for the full feature.

![Delete bucket](./bucket-soft-delete.png)
