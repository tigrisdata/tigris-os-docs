...
```python
def create_bucket_snapshot(tigris, bucket_name, snapshot_name):
    def add_header(request, **kwargs):
        request.headers["X-Tigris-Snapshot"] = f"true; name={snapshot_name}"
    tigris.meta.events.register("before-sign.s3.CreateBucket", add_header)
    try:
        response = tigris.create_bucket(Bucket=bucket_name)
        headers = response.get("ResponseMetadata", {}).get("HTTPHeaders", {})
        return headers.get("x-tigris-snapshot-version")
    finally:
        tigris.meta.events.unregister("before-sign.s3.CreateBucket", add_header)
```
...
### Retrieving Snapshotting and Forking Info for a Bucket

Snapshotting and forking related information for an existing bucket can be
...
- The `X-Tigris-Enable-Snapshot` header will always be present and will be set
  to "true" for snapshotting-enabled buckets.
...
snapshotting is enabled for a bucket:
...
```go
func hasSnapshottingEnabled(ctx context.Context, client *s3.Client, bucketName string) (bool, error) {
...
```python
def has_snapshotting_enabled(tigris, bucket_name):
