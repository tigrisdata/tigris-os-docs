...
At the beginning, the bucket contains two objects (Object 1 with value = `foo`,
Object 2 with value = `bar`).
...
2. **T2** → Object 2 value modified to `quux` using PutObject.
...
9. **T8** → **Bucket C is forked from Bucket A with Snapshot A1**. It starts
   ...
   - Note that both bucket B and C is forked from same parent but as bucket B is
     forked after object 1 got deleted it doesn't inherit object 1 but bucket C
     does.
