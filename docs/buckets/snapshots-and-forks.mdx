...
To convert a Regular bucket to Snapshot, it must not use lifecycle rules or
object expiration (TTL) rules.
...
snapshots are enabled for a bucket:
...
You can create and list snapshots, create a fork, and read objects in a snapshot
if you own the bucket, you are an organization admin, or you have `ReadOnly` or
a higher role on the bucket.

Enabling snapshots on a new or existing bucket requires `Editor` or `Admin`.
