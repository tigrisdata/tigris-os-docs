...
Create a new lifecycle rule. A rule must include a transition
(`--storage-class` with `--days` or `--date`) and/or an expiration
(`--expire-days` or `--expire-date`), and may optionally be scoped via
`--prefix`.
