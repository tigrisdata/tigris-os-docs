...
| `--locations`                 | No       | —       | Bucket location. Options: `global`, `usa`, `eur`, `ams`, `fra`, `gru`, `iad`, `jnb`, `lhr`, `nrt`, `ord`, `sin`, `sjc`, `syd`. Supports comma-separated values for dual region (e.g. `ams,fra`). See [locations docs](https://www.tigrisdata.com/docs/buckets/locations/). |
