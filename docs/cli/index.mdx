...
organizations, the first organization in the list will be selected automatically. You can select a different organization by running `tigris organizations list`.
...
Sessions created with `tigris login credentials` are also temporary and they are cleared when you
run `tigris logout`.
