...
import CliShell from "@site/src/components/CliShell";
...
This is the real CLI in a virtual shell. Run `tigris help` to list the commands,
or `tigris login` to use them on your own buckets.

<CliShell />

## Install

Install the standalone binary. It does not need Node.js.
...
To update the CLI later, run `tigris update`.

<details>
<summary>Other install methods: Homebrew and npm</summary>
...
To update, run `brew upgrade tigrisdata/tap/tigris`.

With npm (needs Node.js):

```bash
npm install -g @tigrisdata/cli
```

To update, run `tigris update`.

</details>

## Authentication

Tigris CLI supports multiple login methods. It allows you to log in via your
...
The quickest way to get started is to log in via your browser (OAuth
authentication):
...
To log in with an access key and secret:
...
commands use these credentials without needing to log in each time.
