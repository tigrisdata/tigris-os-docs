...
Tigris CLI lets you manage your buckets, objects, access keys, and IAM policies
from the terminal.
...
Tigris CLI is now available as standalone binary without the dependency on
nodejs runtime.

:::

{/* prettier-ignore */}
<Tabs groupId="platform" label="Download">
  <TabItem value="linux" label="macOS / Linux">
    ```bash
    curl -fsSL https://raw.githubusercontent.com/tigrisdata/cli/main/scripts/install.sh | sh
    ```
  </TabItem>
  <TabItem value="macos" label="Windows">
    ```bash
