...
Delete one or more objects by key from the given bucket. On a versioned
bucket, the default creates a delete marker; use `--version-id` or
`--all-versions` to hard-delete versions.
