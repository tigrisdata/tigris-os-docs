...
This example allows `s3:GetObject` only for objects in the `nature/mountains`
directory within the `images` bucket, as well as for a specific file,
`pets/Scooby-Doo.jpg`, in the same bucket:
