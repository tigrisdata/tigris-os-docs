...
Shadow buckets move data into Tigris from an external provider. A Tigris bucket
cannot be a shadow bucket source: Tigris rejects the configuration with
`Migration source cannot be Tigris endpoint`. To copy between Tigris buckets,
use a [fork](/docs/buckets/snapshots-and-forks/) or `aws s3 sync`.
