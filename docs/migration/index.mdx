...
Shadow buckets migrate data in from another provider. A Tigris bucket is not a
valid source, and Tigris rejects one with
`Migration source cannot be Tigris endpoint`. To copy between Tigris buckets,
[fork the bucket](/docs/buckets/snapshots-and-forks/) or run `aws s3 sync`.

The shadow bucket source must also be S3-compatible. Azure Blob Storage is not
