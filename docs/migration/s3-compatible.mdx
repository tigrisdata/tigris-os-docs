...
No. The source has to sit outside Tigris, and a Tigris endpoint is rejected with
`Migration source cannot be Tigris endpoint`.
[Fork the bucket](/docs/buckets/snapshots-and-forks/) or run `aws s3 sync` to
copy between Tigris buckets.
