...
```bash
aws s3api --endpoint-url https://t3.storageapi.dev put-object --bucket foo-bucket --key bar-public.txt --body bar.txt --acl public-read
```
...
```bash
aws s3api --endpoint-url https://t3.storageapi.dev put-object --bucket foo-public-bucket --key bar-private.txt --body bar.txt --acl private
```
