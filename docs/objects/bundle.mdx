...
| `x-tigris-bundle-compression` | No       | `none` \| `gzip` \| `zstd` | `none`  |
...
| Value  | Content-Type        | When to use                                      |
| ------ | ------------------- | ------------------------------------------------ |
| `none` | `application/x-tar` | Default. Best for JPEG/PNG (already compressed). |
| `gzip` | `application/gzip`  | Mixed content or bandwidth-constrained links.    |
| `zstd` | `application/zstd`  | Best ratio + speed for large bundles.            |

**Tip:** For image training data, use `none` — double-compressing JPEG/PNG
wastes CPU for negligible size reduction.
...
class TigrisBundleDataset(torch.utils.data.IterableDataset):
    def __init__(self, metadata_path, bucket, batch_size=32, prefetch=20):
        self.bucket = bucket
    ...
    def __iter__(self):
        ...
        for i in range(0, len(my_batches), self.prefetch):
            ...
            response = bundle_objects(s3_client, self.bucket, keys)
            with tarfile.open(fileobj=response, mode="r|") as tar:
                for member in tar:
                    if member.name == "__bundle_errors.json":
                        continue
