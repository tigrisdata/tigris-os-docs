...
| `X-Amz-Copy-Source-If-None-Match`       | Request proceeds only if the source object's ETag does **not** match. Use `If-None-Match: "*"` to write only if the object does not already exist. Returns `412 Precondition Failed` on a matching PUT.                                          |
