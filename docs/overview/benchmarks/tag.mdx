# Model Training on Tigris with TAG Caching

Training ML models directly from object storage works if the data pipeline can
keep up. We [benchmarked](/blog/training-object-storage) Tigris against
...
then tested what changes when you add TAG, a local S3-compatible caching proxy.

## Summary

Tigris reaches ~134 samples/sec at saturation, within 3% of the
...
on the same workload. With TAG's warm cache, warm epochs are **5.7x faster**,
you need **4 workers instead of 16** to saturate the GPU, and data sharding is
**not required**.
...
## 1. Baseline bottlenecks at 16 workers
...
## 2. Sharding data drops workers to 8
...
**Fig. 2.** Throughput is flat across shard sizes. Sharding's benefit is fewer
workers (8 vs 16), not higher throughput.

## 3. TAG Cache drops it to 4 workers
...
| Metric (2 workers)  | Cold (Epoch 1) | Warm (Epoch 2+) |
| ------------------- | -------------- | --------------- |
| Epoch duration      | 4,197s         | 734s            |
| Speedup             | —              | 5.7x faster     |
| Workers to saturate | 16 (no cache)  | 4 (warm cache)  |
...
**Fig. 3.** Epoch duration (seconds) across three epochs. Epoch 1 fills the
cache, so it is slow, and epochs 2 and 3 read from cache and finish in about 735
seconds each.

## 4. TAG: the opposite of bottleneck
...
**Fig. 4.** Raw throughput ceiling: TAG's local cache delivers about 4x more
samples per second than reading directly from Tigris, and storage is not the
bottleneck.
...
- [Full benchmark post](/blog/training-object-storage) — Methodology, shard-size
  sweeps, and per-configuration breakdowns
