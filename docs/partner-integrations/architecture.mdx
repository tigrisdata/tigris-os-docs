...
When a user signs up in your system, you can create their organization and
initial bucket via the API below, or through the
[Partner Dashboard](/docs/partner-integrations/dashboard).
...
The `user_role` field controls what this user can do through management
endpoints. See [Access Control](/docs/partner-integrations/access-control) for
details on how roles and permissions work.

### Offboarding Tenants

To offboard a tenant, deactivate their organization. This immediately revokes
access for all users and access keys. You can deactivate an organization from
the [Partner Dashboard](/docs/partner-integrations/dashboard) or via the API:
...
- **[Access Control](/docs/partner-integrations/access-control)** -- Learn how
  to use `user_role`, `buckets_roles`, and `access_key_scope` to control what
  your tenants' access keys can do
- **[Partner Integrations API Reference](/docs/partner-integrations/api/)** --
  Complete API documentation for all partner endpoints
