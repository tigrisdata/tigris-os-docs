...
give it ReadWrite permissions so the same key can upload your dataset and read
it back during training.
...
- **Batch Size:** Adjust `batch_size` based on your data size and GPU memory.
  ...
  to utilize the GPU efficiently, but not so large that the GPU runs out of
  memory or that data loading becomes a bottleneck.
