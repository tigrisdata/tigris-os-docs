...
The .Net SDK defaults to using v2 signatures, Tigris only supports v4
signatures. In order to use pre-signed URLs, you must set `UseSignatureV4` to
`true` in the global `AWSConfigsS3` object in the Amazon namespace.
