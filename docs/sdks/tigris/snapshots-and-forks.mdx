...
Above example creates a snapshot with the description `pre-finetune` and use
bucket name (in this case `llm-base`) from either `.env` or environment
variables.
...
Above example creates a fork from the bucket using the bucket name (in this case
`llm-base`) from either `.env` or environment variables. It can be overridden
using the `bucketName` parameter in `config`.

```ts
import { createBucketFork } from "@tigrisdata/storage";

await createBucketFork("llm-model-2-fork", {
  config: {
    bucketName: "llm-model-2",
  },
});
```
