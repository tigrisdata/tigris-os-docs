...
if (result.error) {
  ...
  //   path: "object.txt",
  //   size: 12,
  //   url: "https://tigris-example.t3.storageapi.dev/object.txt",
  // }
}
```
...
In case of successful `createBucket`, the `data` property will be set and
contains the following properties:
...
- `sourceBucketSnapshot`: The snapshot version of the source bucket (if this is
  a fork bucket)
...
`getBucketInfo` function can be used to retrieve information about a specific
bucket.
...
In case of successful `getBucketInfo`, the `data` property will be set and
contains the following properties:
...
- `sourceBucketSnapshot`: The snapshot version of the source bucket (if the
  bucket is a fork)
