...
This page covers four patterns where Tigris's architecture fits the access
...
when you need shuffling or indexed lookups. With `enable_sharding=True`, each
DataLoader worker gets a distinct partition automatically.
...
flowchart LR
    ...
    SKY[SkyPilot — any cloud]:::cloud -.->|launches| DL
```
</div>
<div className="mermaid-caption">
  Figure 1: Training data streams from a global bucket through optional TAG
  caching into DataLoader workers, which can run on any cloud via SkyPilot.
</div>

On [benchmarks](/docs/overview/benchmarks/model-training), Tigris reaches ~134
samples/sec at ~99% GPU utilization on a ViT workload (100k JPEGs, g5.8xlarge),
matching AWS S3 on the same setup. Packing images into tar shards cuts the
worker count from 16 to 8 to saturate the GPU. Adding
[TAG (Tigris Acceleration Gateway)](/docs/overview/benchmarks/model-training#3-tag-eliminates-network-latency-after-epoch-1),
a local S3-compatible caching proxy, eliminates network round-trips after
epoch 1: warm epochs run **5.7x faster**, only **4 workers** are needed to
saturate the GPU, and the local cache provides **~200x** throughput headroom
over what the GPU can consume.

For multi-cloud training, [SkyPilot](/docs/quickstarts/skypilot/) spins up GPU
instances on whichever provider has capacity. Because all nodes read from the
same global Tigris bucket at the nearest replica, there are no cross-cloud
egress costs and no per-region storage to manage.
...
## 2. Checkpoint, resume, and fork training runs {#checkpoint-resume-and-fork-training-runs}
...
<div className="mermaid-caption">
  Figure 2: A training job checkpoints into a snapshot, then either resumes from
...
## 3. Trigger post-training pipelines on upload {#trigger-post-training-pipelines-on-upload}
...
[Tigris Object Notifications](/docs/buckets/object-notifications/)
replace the polling loop with a push model. An HTTP `POST` fires to your webhook
the moment a new object lands, carrying the bucket, key, size, and ETag. Your
pipeline handler can start immediately — run evals against the new checkpoint,
kick off ONNX or TensorRT conversion, or trigger a rolling deploy to your
inference fleet.
...
<div className="mermaid-caption">
  Figure 3: A training job uploads weights, triggering a webhook that fans out
...
## 4. Store and serve model weights globally {#store-and-serve-model-weights-globally}
...
the weights available worldwide. Inference nodes read from the nearest replica at
no egress cost. Because weights are immutable per version, conditional
...
<div className="mermaid-caption">
  Figure 4: Training uploads weights once and inference nodes load them from the
  nearest replica.
</div>
