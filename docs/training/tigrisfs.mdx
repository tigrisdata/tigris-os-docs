...
Create a new access key with `ReadWrite` permissions on your target bucket in
the [Tigris Dashboard](https://console.storage.dev/). Configure the aws-cli to
utilize the credentials as described in the guide
[here](/docs/sdks/s3/aws-cli/).
...
TigrisFS uses a cache for both read buffers when it needs to read data from your
...
a bucket with a 15-minute metadata cache TTL:
